...
function readToolMessage(){
	var toolMessage = sessionStorage.getItem("toolMessage");
	if(toolMessage != null && toolMessage != ""){
		ToolTipTop.Show(toolMessage,"success");
	};
	sessionStorage.removeItem("toolMessage");
}
/**
 * 表单元素序列化
 */
		...
		if(Math.ceil(totalCount / pageSize) < 10) {
			for(var i = 1; i <= Math.ceil(totalCount / pageSize); i++) {
				...
				if(pageIndex === page_index_current) {
					pagination_html += "<li class='active'><a href=\"javascript: "+fn+"(" + pageIndex + ");\">" + pageIndex + "</a></li>";
				} else {
					pagination_html += "<li><a href=\"javascript: "+fn+"(" + pageIndex + ");\">" + pageIndex + "</a></li>";
				}
			}
		} else {
			for(var i = 1; i <= 10; i++) {
...
//var alr = $(['<div id="messager-dialogue" class="modal fade" style="margin-top:100px;">', '<div class="modal-dialog modal-sm">', '<div class="modal-content" style="position: relative;">', '<div class="modal-header" style="padding-bottom:5px;font-family: \"微软雅黑\";">', '<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true" style="position: absolute;top:5px;right:8px;">×</span>', '<span class="sr-only">Close</span>', '</button>', '<h4 class="modal-title" style="color:#7b7b7b">', '[Title]', '</h4>', '</div>', '<div class="modal-body small">', '<div class="row">', '<div class="col-xs-12 center-block">[MessagerIcon]</div>', '<div class="col-xs-12 center-block messageText" style="margin-top:20px;font-size:16px;margin-bottom:30px">[Message]</div>', '<div class="col-xs-12 center-block">','<button type="button" class="ok" data-dismiss="modal">[BtnOk]</button>', '<button type="button" class="cancel" data-dismiss="modal">[BtnCancel]</button>','</div>','</div>', '</div>', '</div>', '</div>' , '</div>'].join(''));
/**
 * 加载用户名
 */
