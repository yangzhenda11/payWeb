...
function signIns() {
	...
	if(userName == "" && !passwords == ""){
		Messager.show({Msg: '用户名不能为空',iconImg: 'warning', isModal: false});
	};
	if(passwords == "" && !userName == ""){
		Messager.show({Msg: '密码不能为空',iconImg: 'warning', isModal: false});
	};
	if(passwords == "" && userName == ""){
		Messager.show({Msg: '用户名不能为空',iconImg: 'warning', isModal: false});
	};
	if(!passwords == "" && !userName == ""){
		...
		$.ajax({
				        	...
				        	if(result.code == 200){
				        		...
				        		for(var i in result){
				        			if(result[i].topBarCode != "testing"){
				        				var obj = {"topBarCode":result[i].topBarCode,"topBarName":result[i].topBarName};
				        				data.push(obj);
				        			}
				        		};
				        		/*
				        		 ...
				        		 */
					        	if(data.length == 0){
					        		Messager.show({Msg: "您没有进入系统的权限！",iconImg: 'warning', isModal: false});
			...
			error:function(){
				Messager.show({Msg: "连接服务器失败",iconImg: 'warning', isModal: false});
			}
		});
	};
};
	...
	$.ajax({
        ...
        success: function (result) {
			...
			if (result.code === 200 && data != null) {
    			...
    			Messager.show({Msg: '暂无数据',iconImg: 'warning', isModal: false});
    		}	        		
        }
  	});
  	return userUrl;
